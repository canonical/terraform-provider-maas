...
- `system_id` (String) The machine system ID. Looking up a machine by its system ID is significantly faster than by `hostname` or `pxe_mac_address`, which require a lookup over all machines.
