...
func TestAccStaticRoute_basic(t *testing.T) {
	// We define some resources to use during the test
	// most of this is similar to `resource_maas_subnet_ip_range_test.go`
	// we provide the default VLAN and fabric to the test
	fabricID := "0"
	VLANID := "0"
	...
	baseConfig := testAccGenerateSubnet(sourceCIDR, sourceGateway, fabricID, VLANID, sourceName) +
		testAccGenerateSubnet(destinationCIDR, destinationGateway, fabricID, VLANID, destinationName) +
		testAccGenerateSubnet(changedCIDR, changedGateway, fabricID, VLANID, changedName)
		...
		Steps: []resource.TestStep{
			// Test the resource can be created
			{
				...
				Check: resource.ComposeTestCheckFunc(
					// first check the rsource exists
			...
			// And finally, we need to test the import functionality
			{
				...
				ImportStateIdFunc: func(s *terraform.State) (string, error) {
					rs, ok := s.RootModule().Resources["maas_static_route.test"]
					if !ok {
						return "", fmt.Errorf("resource not found: maas_static_route.test")
					}
...
func testAccGenerateSubnet(cidr string, gateway string, fabric string, vlan string, name string) string {
	return fmt.Sprintf(`
...
`, name, cidr, gateway, fabric, vlan, name)
}
...
func testAccCheckMAASStaticRouteDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		// error 404 means the route wasn't found, and the resource deleted
